...
def build_token_to_id_vocab(sentences, specials=('<pad>', '<bos>', '<eos>', '<unk>')):
    # TODO: build a token-to-id dict with specials first, then corpus tokens in first-seen order.o
...
def build_id_to_token_vocab(token_to_id):
    ...
    build_id = {idx: token for token, idx in token_to_id.items()}
    return build_id
    pass
...
def encode_sentence_to_ids(sentence, token_to_id, unk_token='<unk>'):
    ...
    unk_token = token_to_id[unk_token]
...
def decode_ids_to_tokens(ids, id_to_token):
    ...
    return [id_to_token.get(id) for id in ids]
    pass
